...
class GeometryConnector:
    def __init__(self):
        ...
        self.threshold = bpy.context.scene.connected_edge_angle_threshold
        self.area_th = scene.area_threshold
        self.edge_th = scene.edge_threshold


    # Сравнение нормалей граней с учётом выравнивания совпавших рёбер
    def compare_normals(self, f1: Face, f2: Face, matching_edges: List[Tuple[Edge, Edge]]) -> bool:
        # Порог и его косинус
        cos_th = math.cos(self.threshold)
            ...
            for m2 in pieces_meshes[i + 1:]:
                for f1 in m1.faces:
                    ...
                    for f2 in m2.faces:
                        ...
                        # Сравнение площадей
                        if not math_utils.compare_values(a1, f2.area, self.area_th):
                            coeff -= AREA_PENALTY
                        ...
                        for e1 in edges1:
                            ...
                            for e2 in edges2:
                                if e2.new_index in used:
                                    continue
                                if math_utils.compare_values(e1.length, e2.length, self.edge_th):
                        ...
                        if len(matched_edges) > 2:
                            if not self.compare_normals(f1, f2, matched_edges):
